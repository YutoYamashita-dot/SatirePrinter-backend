...
export default async function handler(req) {
  try {
    ...
    const styleLine =
      "スタイル＝文語体・書き言葉。喋り口調は禁止。文末は「だ／である」調で断定する。三点リーダ「…」や疑問符・感嘆符は禁止。";
    ...
    const userMsg =
`${langLine}
次の「言葉」について、${lengthRule}の鋭く辛辣な風刺/皮肉を${langName}で作成すること。難解語は避け、**必ず書き言葉（文語体・断定調）で出力すること**。喋り言葉・会話調・独白は禁止。
${styleLine}
追加要件:
